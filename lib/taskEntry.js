...
module.exports = function (thorin, opt) {
  ...
  class TaskEntry {
     ...
     *  - recurrent: true / repeat the task
     *  - timeout
     * */
    ...
    createContext(actionNames, opt) {
      ...
      function onContextTerminated() {
        if (_terminateTimer) clearTimeout(_terminateTimer);
        // we try to delay the re-execution 1s to avoid entering an recurring loop.
        _terminateTimer = setTimeout(() => {
          _terminateTimer = null;
          contextObj.destroy();
          isDone = true;
          if (typeof opt.timer === 'undefined' && opt.recurrent !== true) return;
          let nextDelay = getSeconds(contextObj.delay()) * 1000;
          let timerMs = (opt.timer ? opt.timer * 1000 : 0);
          timerMs += nextDelay;
